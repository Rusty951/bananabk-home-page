...
- `analytics.js`는 운영 도메인에서만 GA4를 초기화하고, 카카오톡 CTA 클릭의 UTM/버튼 위치 기록과 `/food-photo`의 Meta `PageView`/`Contact` 이벤트를 담당한다. Meta Pixel도 운영 도메인에서 `public-config.js`에 실제 ID가 있을 때 `/food-photo`에서만 로드한다.

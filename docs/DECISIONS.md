...
## 2026-07-21: 운영 도메인에서만 분석 도구를 초기화한다

- 결정: HTML별 GA4 인라인 초기화를 제거하고 `analytics.js`로 통합하며, GA4와 Meta Pixel은 `bananabk2425.com`, `www.bananabk2425.com`에서만 로드한다.
- 이유: 커스텀 이벤트 차단과 별개로 HTML의 기본 PageView가 localhost와 `127.0.0.1`에서 먼저 발송돼 운영 데이터에 섞였다.
- 영향: 적용 이후 로컬·Vercel 미리보기 트래픽은 수집하지 않고 운영 방문과 문의 행동만 분석한다.

## 2026-07-19: 푸드 촬영 광고 랜딩 기본 상품과 측정 기준 확정

결정:

- `/food-photo`는 `30만 원부터`, 대표 메뉴 최대 5종, 기본 보정본 10컷 기준을 공개 기본 구성으로 표시한다.
- 촬영 지역과 납기는 랜딩에 공개하지 않고 광고 타기팅과 상담에서 관리한다.
- 카카오톡 CTA는 GA4에 랜딩 UTM과 버튼 위치를 기록하고, Meta Pixel ID 설정 시 `/food-photo`에서만 표준 `PageView`와 `Contact` 이벤트를 기록한다.
- `/food-photo`에는 Meta 광고 측정 안내와 외부 활동 관리 링크를 표시하고, 이름·전화번호·문의 내용은 Pixel 이벤트로 전송하지 않는다.

이유:

- 가격만 보이고 제공 범위가 모호한 상태를 줄이면서 추가 메뉴와 촬영 범위에 따른 견적 여지를 남긴다.
- 광고 유입의 소재·캠페인별 문의 클릭을 비교할 수 있어야 한다.
...
- 저장소 check 명령을 실행한다.
- 로컬 정적 서버에서 `/food-photo`의 데스크톱과 360/390/430px 화면을 확인한다.
